...
namespace BrewDay2.Controllers
{
    ...
    public class RicetteController : Controller
    {
        ...
        {
            var lista = !String.IsNullOrEmpty(nomeparametro) ? _db.Ricette.Where(x => x.Categoria == nomeparametro && (x.Privata == false || x.UserId==User.Identity.GetUserId())).ToList() : _db.Ricette.ToList();
